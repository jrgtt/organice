...
the Dropbox developer console]], create a new app, select "Full Dropbox"
as access type and name it accord to your choice. Once inside the
configuration page copy the *App key* and in a newly created ~.env~
file (analogous to ~.env.sample~) set it as value for
...
To be able to read and modify your files it's necessary to allow the
following permissions for your app: ~account_info.read~,
